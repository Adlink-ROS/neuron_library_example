...
/**/
    ...
    : Node("gpio_server", options)
    {
            ...
            std::shared_ptr<neuronlib_interfaces::srv::GpioService::Response> response)-> void         
        {
            ...
            if (gpio_pin[request->gpio_num]==NULL)//[request->gpio_num]
            {
                gpio_pin[request->gpio_num] = mraa_gpio_init(request->gpio_num);//[request->gpio_num]
            ...
            {
                if (mraa_gpio_write(gpio_pin[request->gpio_num], (request->value == 0)?0:1) != MRAA_SUCCESS) //[request->gpio_num]
                {
            ...
            {
                int pin_val = mraa_gpio_read(gpio_pin[request->gpio_num]);//[request->gpio_num]
                if (pin_val == -1)
                {
